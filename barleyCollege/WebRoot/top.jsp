	...
	<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
		...
		<ul class="nav navbar-nav navbar-right">
			...
			<a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true"
				aria-expanded="false">
					欢迎${username} <span class="caret">
					</span>
			</a>
				<ul class="dropdown-menu">
					<li>
						<%String sf=(String)session.getAttribute("shenfen");%>
						<% if("学生".equals(sf)){%>
						<a href="<%=request.getContextPath()%>\student.jsp">个人主页</a>
						<%}else if("老师".equals(sf)){ %>
						<a href="<%=request.getContextPath()%>\teacher.jsp">个人主页</a>
						<%}else{}%>
					</li>	
					<%if(session.getAttribute("username")!=null){ %>
					<li><a href="<c:url value="publish.jsp"/>">发表博客</a></li>
					<li><a href="">修改密码</a></li>
					<li role="separator" class="divider"></li>
					<li><a href="<c:url value="UserServlet?method=exit"/>" onclick="exit()">退出登录</a></li>
					<%} %>
		</ul>
	</div>
</div>
